...
home_variables=$(grep -oP '(?<=export ).*?_HOME=.*' ~/.bashrc)
...
select selected_option in $home_variables; do
    if [[ -n $selected_option ]]; then
        # Extract the variable name
        VAR_NAME="${selected_option%%=*}"
        # Get the value of the variable
        APEX_TOP_LEVEL_FOLDER="${!VAR_NAME}"
        break
    fi
done
...
echo "Script execution completed. Please note the following:"
echo -e "If you are running multiple instances on the same server, you'll need to \e[1mmanually change these to unused port numbers:\e[0m"
...
echo "You will need to start the service manually the first time using the following command:"
